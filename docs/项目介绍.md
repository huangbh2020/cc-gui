...
> **一句话定位**:基于 Claude Agent SDK 构建的桌面端三栏 IDE,**不重新实现 agent**,只提供完整的交互表面:多会话实时对话、Monaco 文件编辑器、多仓库 Git 管理、可深度定制的集成终端、内置可拾取元素的浏览器,以及 LSP、模型切换、AI 生成提交信息、AI 解决合并冲突等"AI × IDE"组合能力。
...
- **输入体验**:输入框支持附加文件、粘贴图片、斜杠命令,直接拖拽文件树里的文件也可以送入对话。
...
- **Monaco 编辑器**:享受 VS Code 同款的语法高亮、智能提示、缩进、查找替换。
- **多语言支持**:`FileEditor.languageForExt()` 覆盖了 30+ 常见语言,包括:
...
- **三态文件视图**:
  ...
  - **Preview**:Markdown 渲染(Shiki 代码高亮 + GFM + 数学公式)、图片预览(透明 PNG 用棋盘背景展示,点击切换 1:1 缩放)、二进制文件(Office / 压缩包 / 字体 / PDF 等)给出友好的"无法预览,可在系统中打开"提示。
- **脏标记与保存指示**:文件改动时,tab 上显示圆点;保存过程中底部弹出"保存中 / 已保存 / 保存失败"轻量 toast。
- **OpenTabsBar 模式切换**:支持"多标签页"与"单文件替换"两种编辑器打开模式,任意时点都能切回 tabs。
- **Reveal 跨文件跳转**:在文件树中选中一个文件时,自动展开父目录链、`scrollIntoView` 该节点(用 rAF 轮询,容忍目录懒加载的延迟)。
- **右键上下文菜单**:对文件/目录统一提供「在资源管理器中显示」「复制绝对路径」「复制相对路径」「添加到聊天」「打开」等动作,操作后还会浮出一个 "已复制" 提示气泡。
- **拖拽到对话**:文件节点 `draggable` 写入自定义 MIME,Composer 接收后直接作为附件加入当前会话。
- **小图标(agent-touched 标记)**:本轮 agent 新建的文件显示 accent 圆点,改动的文件显示 info 圆点,一眼看出 Claude 干了啥。

### 3.2 LSP 集成(把语言服务器跑起来)
...
- 主进程同步调 Chromium 的 device emulation(视口 + 触摸 + UA),Renderer 端把 view 的 bounds 收窄到对应设备宽度,**叠加一个真正的移动端渲染环境**,而不只是把窗口变窄。
- `syncBounds` 用 `ResizeObserver` + window `resize` 双监听,所有变化走 rAF 节流;切换设备时还会清空 `lastBoundsRef` 让新尺寸穿透去重检查。
